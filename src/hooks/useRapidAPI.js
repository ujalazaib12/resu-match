...
// Mock Data for Pakistani Jobs (Fallback)
const MOCK_JOBS = [
	{
		id: 101,
		...
		contents:
			"<p>We are looking for a Senior Software Engineer with expertise in .NET and Angular...</p>",
	...
	},
	{
		...
		contents:
			"<p>Arbisoft is hiring a talented MERN Stack Developer to join our dynamic team...</p>",
		publication_date: new Date().toISOString(),
		type: "Full Time",
	},
	{
		...
		contents:
			"<p>Join 10Pearls as an SQA Engineer. Experience with automation testing is a plus...</p>",
		publication_date: new Date().toISOString(),
		type: "Full Time",
	},
	{
		...
		levels: [{ name: "Senior Level" }],
		contents:
			"<p>Sadapay is looking for a Product Manager to lead our fintech innovations...</p>",
		publication_date: new Date().toISOString(),
		type: "Full Time",
	},
	{
		...
		levels: [{ name: "Mid Level" }],
		contents:
			"<p>Contour Software is seeking a React Native Developer for mobile app development...</p>",
		publication_date: new Date().toISOString(),
		type: "Full Time",
	},
	{
		...
		levels: [{ name: "Mid Level" }],
		contents:
			"<p>Folio3 needs a creative UI/UX Designer with a strong portfolio...</p>",
		publication_date: new Date().toISOString(),
		type: "Contract",
	},
];

export const useRapidAPI = () => {
	...
	console.log("RapidAPI Key loaded:", !!RAPID_API_KEY);

	const searchJobs = async (params) => {
		// Create a unique cache key based on search params
		const cacheKey = `jsearch_${JSON.stringify({
			query: params.query,
			location: params.location,
			page: params.page,
		})}`;

		// 1. Try Cache First (if RAPID_API_KEY is present, we try to be smart about it)
		if (RAPID_API_KEY) {
			...
			if (cached) {
				...
				if (Date.now() - timestamp < 86400000) {
					console.log("Using Cached Data:", data.results?.length, "items");
					return {
		...
		// 2. Try RapidAPI
		if (RAPID_API_KEY) {
			console.log("Using RapidAPI/JSearch...");
			try {
				const options = {
					method: "GET",
					url: "https://jsearch.p.rapidapi.com/search",
					params: {
						query: `${params.query || "Software Engineer"} in ${
							params.location || "Pakistan"
						}`,
						page: params.page || "1",
						num_pages: "1",
					},
				...
				if (response.data && response.data.data) {
					apiResults = response.data.data.map((job) => ({
						id: job.job_id,
						name: job.job_title,
						company: { name: job.employer_name, logo: job.employer_logo },
						locations: [
							{ name: `${job.job_city || ""}, ${job.job_country || ""}` },
						],
				...
				console.warn(
					"JSearch API Error (Falling back to mock data):",
					error.message
				);
				// Execution naturally continues to fallback below
			}
		}

		// If API call was successful and returned data, save to cache and return
		if (apiResults) {
			const resultData = {
				results: apiResults,
				page_count: 5,
		...
		// 3. FALLBACK: Mock Data Filtering
		...
		return new Promise((resolve) => {
			setTimeout(() => {
				console.log("Mock Job Count:", filtered.length);
				resolve({
					results: filtered,
					page_count: 1,
					page: 1,
			...
			try {
				...
				if (jobData) {
					return {
						...
						locations: [
							{
								name: `${jobData.job_city || ""}, ${jobData.job_country || ""}`,
							},
						],
